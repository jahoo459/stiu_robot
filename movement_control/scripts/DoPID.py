...
# output:
# motor_cmd - the output of the PID controller, the power going to the motor
# wheel_vel - the current velocity of the wheel in mm/second

# input:
# real_wheel_vel - the velocity from the encoder in mm/second
# wheel_vtarget - the desired velocity in mm/second

# parameters:
# Kp, Ki, Kd (proprtional, integral, derivative gain to the PID controller)
# out_min/out_max - min/max output of the wheel_vel
# rate - The frequency at which to publish the wheel_vel topic (in Hz)

## do it different!
# timeout_ticks - If no wheel_vtarget messages are received in timeout_ticks ticks, the wheels are stopped. (I.E. timeout_ticks / rate seconds) 
...
######################################################
    ...
    def __init__(self):
    #####################################################
        rospy.init_node('DoPID', anonymous=True)
